...
echo "Starting recursive image file renaming..."
...
find . -type f | while IFS= read -r old_path; do
    ...
    # Normalize extension to lowercase for reliable image checking
    lower_ext=$(echo "$ext" | tr '[:upper:]' '[:lower:]')
    
    # Target all common image formats (Raster, Camera RAW, Vector, etc.)
    case "$lower_ext" in
        png|jpg|jpeg|webp|gif|bmp|tiff|tif|svg|ico|heic|heif|raw|cr2|nef|orf|sr2|psd|ai|avif)
...
echo "------------------------------------------"
echo "Done renaming image files!"